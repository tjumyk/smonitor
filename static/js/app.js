...
(function() {
  ...
  parse_error_response = function(response) {
  ...
  app.controller('RootController', [
    ...
    $interval) {
      var format_cpu_time,
    handle_update_result_message,
    init,
    process_full_status_message,
    process_info_message,
    process_proccess_info,
    ...
    time;
        ...
        if (info.npu_memory !== void 0) {
          return info.npu_memory_h = human_size(info.npu_memory);
        }
      };
    ...
    output;
      ...
      handle_update_result_message = function(host,
    message) {
    ...
    function(response) {
              $scope.server_updating = false;
              if (response.status === 401 && response.data.redirect_url) { // oauth redirect
    ...
    function() {
          return $timeout(function() {
            ...
            return $scope.init_success = true;
          });
        });
    ...
    function() {
          $('.reconnect.dimmer').dimmer({
            'closable': false
          }).dimmer('set page dimmer',
    true).dimmer('show');
          return $http.get('api/config').then(function(response) {
            if (!angular.equals(raw_config,
    response.data)) {
              return window.location.reload();
            }
          },
    ...
    status_message;
          ...
          socket.on('full_status',
    function(message) {
            return $timeout(function() {
              var full_status_message,
    name,
    results;
              results = [];
              for (name in message) {
    ...
    results;
              ...
              for (name in message) {
                ...
                results.push(handle_update_result_message(host_map[name],
    result_message));
              }
              return results;
            });
  ...
  app.controller('HomeController', [
    ...
    $timeout) {
      return $timeout(function() {
        return $('.init-box').addClass('active');
      },
    100);
    }
  ]);
  ...
  app.controller('HostController', [
    ...
    re_enable_full_status;
      ...
      re_enable_full_status = function() {
        return $scope.socket.emit('enable_full_status',
    host_id);
      };
      $scope.$on('$destroy',
    function() {
        if ($scope.socket && $scope.host) {
          $scope.socket.off('reconnect',
    re_enable_full_status);
          $scope.socket.emit('disable_full_status',
    host_id);
        }
        if ($scope.host) {
    ...
    ref1;
        ...
        socket.emit('enable_full_status',
    host_id);
        return socket.on('reconnect',
    re_enable_full_status);
      });
      return $scope.update = function() {
        if ($scope.host) {
          ...
          return $scope.socket.emit('update',
    $scope.host.name);
        }
      };
    }
